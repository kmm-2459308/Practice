...
// 重複データを削除
void Purge(DblList* list, int compare(const Member* a, const Member* b))
{
	// ここをコーディングしてください
	if (isEmpty(list) || list->head->next == list->head) 
	{
		return;
	}

	DblNode* current = list->head->next;

	while (current != list->head) 
	{
		DblNode* runner = current->next;

		while (runner != list->head) 
		{
			if (compare(&current->data, &runner->data) == 0) 
			{
				DblNode* temp = runner;
				runner = runner->next;
				Remove(list, temp);
...
// N番目のノードを取得
DblNode* Retrieve(const DblList* list, int no)
{
	// ここをコーディングしてください
	if (isEmpty(list)) 
	{
	...
	DblNode* head = list->head;
	DblNode* current = head->next;
	int count = 1;

	while (current != head && count < no) 
	{
		current = current->next;
		count++;
	}
